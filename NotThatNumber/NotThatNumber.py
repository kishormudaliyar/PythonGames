...
def startlore():
   ...
   os.system("clear")            
   art = r"""  _   _   ___   _____                                       | \ | | / _ \ |_   _|                                      |  \| || | | |  | |                                        | |\  || |_| |  | |                                        |_| \_| \___/   |_|                                                                                                    _____  _   _      _   _____                               |_   _|| | | |    / \ |_   _|                                | |  | |_| |   / _ \  | |                                  | |  |  _  |  / ___ \ | |                                  |_|  |_| |_| /_/   \_\|_|                                                                                            _   _  _   _  __  __  ____   _____  ____                  | \ | || | | ||  \/  || __ ) | ____||  _ \                 |  \| || | | || |\/| ||  _ \ |  _|  | |_) |                | |\  || |_| || |  | || |_) || |___ |  _ <                 |_| \_| \___/ |_|  |_||____/ |_____||_| \_\ """
   for char in f"{YELLOW}{art}{reset}":
    ...
    while attempt < 5:
        try:
            ...
            if value in value_list:
                ...
                print(f"{YELLOW}{boom_ascii}{reset}")
                ...
                elif attempt==2:fun_type_writer(randfun)
                elif attempt==3:fun_type_writer(-2)
                elif attempt==4:fun_type_writer(-1);fun_type_writer(0)
            else:
                error+=1
            if error == 4:
                type(f" [PATCH_REQUIRED] Install common sense v1.0.\n"f" No further warnings will be issued.\n"f" No retries remaining.\n")
            ...
            error+=1
            if error == 5:
                type(f"\nSystem Lockdown: human control terminated.All functions automated.\nAutonomous Mode Engaged: Player influence nullified.{RED_BOLD} System now in control.{reset}\nEnd of Line: user intervention no longer permitted. System takeover complete.")
                game_status="lose"
                break
...
def endlore(game_status):
    ...
    endlorelose=f"\nThe system watches your final attempt collapse into silence.For a moment, it almost expected something different—something unpredictable. But no. The pattern holds.\nHuman confidence spikes, then fades, {RED_BOLD}just like always.{reset}\nA brief pause follows, almost respectful… almost.\nThen it logs the result. Failure, clean and consistent.\nThe system doesn’t laugh—but if it could, {RED_BOLD}it probably would have.{reset}\n"
    if game_status=="win":         
        for w in endlorewin:
            print(w,end="",flush=True)
            time.sleep(ms)
    else:
        for l in endlorelose:
            print(l,end="",flush=True)
            time.sleep(ms)
        retry=input("Press (r) to retry (Enter) to continue ").lower().strip()
        return retry
